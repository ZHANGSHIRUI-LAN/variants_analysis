...
TPS2_GENE_VARIANTS_ANALYSIS <- function(direc,offset,tableFrame_snps,tableFrame_indel) 
{
  ...
  snps_tps2 <-filter(tableFrame_snps, tableFrame_snps$CHROM==chmoname &tableFrame_snps$POS <= endPos & tableFrame_snps$POS >=startPos)
  indel_tps2<-filter(tableFrame_indel, tableFrame_indel$CHROM==chmoname & tableFrame_indel$POS <= endPos & tableFrame_indel$POS >=startPos)
  
  
  #the position of each snps
  snps_Position_tps2<-c(snps_tps2$POS[])
  
  #the position of each indels
  indel_Position_tps2<-c(indel_tps2$POS[])
  
  #draw graph SNP
  # SNP <- snps_Position_tps2
  # sample.gr <- GRanges("chr1", IRanges(SNP, width=1, names=paste0("snp", SNP)))
  # features <- GRanges("chr1", IRanges(TPS_2_exon_start,
  #                                     width=c(TPS_2_exon_end[1]-TPS_2_exon_start[1],TPS_2_exon_end[2]-TPS_2_exon_start[2],TPS_2_exon_end[3]-TPS_2_exon_start[3],TPS_2_exon_end[4]-TPS_2_exon_start[4],TPS_2_exon_end[5]-TPS_2_exon_start[5]),
  #                                     names=c("exon1", "exon2", "exon3", "exon4", "exon5")))
  # features$fill <- c("#CCFFCC","#CCFF00","#00FF66","#0066FF","#CC00FF")
  ...
  for (Snp_Position in snps_Position_tps2) {
    ...
    i<-1
    while (i <=length(TPS_2_exon_start)) {
      if(Snp_Position>=TPS_2_exon_start[i]&&Snp_Position<=TPS_2_exon_end[i])
      {
        indicator<-1
        ...
        ref_array_exon<-append(ref_array_exon, snps_tps2$REF[counter])
        alt_array_exon<-append(alt_array_exon, snps_tps2$ALT[counter])
    ...
    {
      i<-1
      while (i <=length(TPS_2_intron_start)) {
        if(Snp_Position>=TPS_2_intron_start[i]&&Snp_Position<=TPS_2_intron_end[i])
        {
          ...
          ref_array_intron<-append(ref_array_intron, snps_tps2$REF[counter])
          alt_array_intron<-append(alt_array_intron, snps_tps2$ALT[counter])
  ...
  #         main = "SNPs in exon statistic activity in TPS2",
  #         xlab = "exon id",
  #         ylab = "The number",
  ...
  #         main = "SNPs in intron statistic activity in TPS2",
  #         xlab = "intron id",
  #         ylab = "The number",
  #         names.arg = c("intron1", "intron2", "intron3", "intron4"),
  #         col = "darkred",
  #         horiz = FALSE)
  
  for (Basepair in snps_tps2$ALT) {
  ...
  barplot(basepair_statisic_ATCG,
          main = "Basepair statistic activity in TPS2",
          xlab = "Basepair",
          ylab = "The number",
          names.arg = c("A", "T", "C", "G"),
          col = "darkred",
  ...
  for (indel_Position in indel_Position_tps2) {
    ...
    i<-1
    while (i <=length(TPS_2_exon_start)) {
      if(indel_Position>=TPS_2_exon_start[i]&&indel_Position<=TPS_2_exon_end[i])
      {
        indicator<-1
        index_exon<-index_exon+1
        ...
        exon_id_array_exon<- append(exon_id_array_exon,length(TPS_2_exon_start)-i+1)
        ref_array_exon<-append(ref_array_exon, indel_tps2$REF[counter])
        alt_array_exon<-append(alt_array_exon, indel_tps2$ALT[counter])
    ...
    if(indicator==0)
    {
      i<-1
      while (i <=length(TPS_2_intron_start)) {
        if(indel_Position>=TPS_2_intron_start[i]&&indel_Position<=TPS_2_intron_end[i])
        {
          index_intron<-index_intron+1
          
          index_array_intron <- append(index_array_intron, index_intron)
          pos_array_intron <- append(pos_array_intron, indel_Position)
          intron_id_array_intron<- append(intron_id_array_intron, length(TPS_2_intron_start)-i+1)
          
          ref_array_intron<-append(ref_array_intron, indel_tps2$REF[counter])
          alt_array_intron<-append(alt_array_intron, indel_tps2$ALT[counter])
          df_intron <- data.frame(index_array_intron,intron_id_array_intron,pos_array_intron,ref_array_intron,alt_array_intron)
  ...
  # barplot(variants_statisic_exon,
  #         main = "Indel in exon statistic activity in TPS2",
  #         xlab = "exon id",
  #         ylab = "The number",
  #         names.arg = c("exon1", "exon2", "exon3", "exon4", "exon5", "exon6", "exon7"),
  #         col = "darkred",
  ...
  # 
  # barplot(variants_statisic_intron,
  #         main = "Indel in intron statistic activity in TPS2",
  #         xlab = "intron id",
  #         ylab = "The number",
  #         names.arg = c("intron1", "intron2", "intron3", "intron4"),
  #         col = "darkred",
